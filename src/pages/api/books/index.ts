...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function createBook() {
    ...
    const body = { ...req.body };
    if (body?.reading_history?.length > 0) {
      const create_reading_history = body.reading_history;
      body.reading_history = {
        create: create_reading_history,
      };
    } else {
      delete body.reading_history;
    }
